...
def do_get_account_info(resource: str, headers: Dict[str, str]) -> http_responses.OK:
    ...
    try:
        acct = bank.get_account(int(params['account_num']), user_token)
...
def do_create_account(headers: Dict[str, str], request_body: str) -> http_responses.OK:
    ...
    try:
        ...
        acct = bank.create_account(params['owner_name'], balance, user_token)
        # Convert the returned Account object to JSON and wrap in an OK response
        return http_responses.OK(bleach.clean(json.dumps(vars(acct), indent=4)))#cleaned
...
def do_deposit(headers: Dict[str, str], request_body: str) -> http_responses.OK:
    ...
    try:
        # Call the bank.py logic and handle raised exceptions
        acct = bank.deposit(int(params['account_num']), amount, params.get('notes', ''), user_token)
        # Convert the returned Account object to JSON and wrap in an OK response
        return http_responses.OK(bleach.clean(json.dumps(vars(acct), indent=4)))#cleaned
    except (TypeError, ValueError) as err:
        raise http_responses.BadRequest(err)
...
def do_withdraw(headers: Dict[str, str], request_body: str) -> http_responses.OK:
    ...
    try:
        # Call the bank.py logic and handle raised exceptions
        acct = bank.withdraw(int(params['account_num']), amount, params.get('notes', ''), user_token)
        # Convert the returned Account object to JSON and wrap in an OK response
        return http_responses.OK(bleach.clean(json.dumps(vars(acct), indent=4)))#cleaned
    except (TypeError, ValueError) as err:
        raise http_responses.BadRequest(err)
...
def do_login(headers: Dict[str, str], request_body: str) -> http_responses.OK:
    ...
    try:
        user = bank.login(params['username'], params['password'])
    ...
    with socket.socket() as sock:
        ...
        while True:
            ...
            try:
                ...
                if client_address[0] not in client_request_rates:
                    client_request_rates[client_address[0]] = {"time": time.time(), "attempts": 1}
                else:
                    if time.time() > client_request_rates[client_address[0]] ["time"] + 60:
                        client_request_rates[client_address[0]] = {"time":time.time(), "attempts":1}
                    else:
                        if client_request_rates[client_address[0]] ["attempts"] > 60:
                            raise http_responses.TooManyRequests("Too many requests")
                        else:
                            client_request_rates[client_address[0]] ["attempts"] += 1
                ...
                # Pull apart the HTTP request into it's constituent parts.
